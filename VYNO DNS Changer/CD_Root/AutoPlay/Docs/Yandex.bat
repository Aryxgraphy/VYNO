...
netsh interface ip set dns name="%adapter%" source=static addr=77.88.8.8 >nul 2>&1
if %errorlevel% equ 0 (
    echo IPv4 DNS set to 77.88.8.8 for %adapter%.
    echo IPv4 DNS set to 77.88.8.1 for %adapter%. >> "%logfile%"
) else (
    echo Failed to set IPv4 DNS for %adapter%.
    echo Failed to set IPv4 DNS for %adapter%. >> "%logfile%"
)
netsh interface ip add dns name="%adapter%" addr=77.88.8.1 index=2 >nul 2>&1
if %errorlevel% equ 0 (
    echo IPv4 Secondary DNS set to 77.88.8.8 for %adapter%.
    echo IPv4 Secondary DNS set to 77.88.8.1 for %adapter%. >> "%logfile%"
) else (
