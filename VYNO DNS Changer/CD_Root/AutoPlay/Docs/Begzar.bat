...
netsh interface ip set dns name="%adapter%" source=static addr=185.55.225.25 >nul 2>&1
if %errorlevel% equ 0 (
    echo IPv4 DNS set to 185.55.225.25 for %adapter%.
    echo IPv4 DNS set to 185.55.226.26 for %adapter%. >> "%logfile%"
) else (
    echo Failed to set IPv4 DNS for %adapter%.
    echo Failed to set IPv4 DNS for %adapter%. >> "%logfile%"
)
netsh interface ip add dns name="%adapter%" addr=185.55.226.26 index=2 >nul 2>&1
if %errorlevel% equ 0 (
    echo IPv4 Secondary DNS set to 185.55.225.25 for %adapter%.
    echo IPv4 Secondary DNS set to 185.55.226.26 for %adapter%. >> "%logfile%"
) else (
