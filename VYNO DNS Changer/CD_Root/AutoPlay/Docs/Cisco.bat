...
if %errorlevel% equ 0 (
    ...
    echo IPv4 DNS set to 208.67.222.20 for %adapter%. >> "%logfile%"
) else (
    echo Failed to set IPv4 DNS for %adapter%.
    echo Failed to set IPv4 DNS for %adapter%. >> "%logfile%"
)
netsh interface ip add dns name="%adapter%" addr=208.67.222.20 index=2 >nul 2>&1
if %errorlevel% equ 0 (
    echo IPv4 Secondary DNS set to 208.67.222.222 for %adapter%.
    echo IPv4 Secondary DNS set to 208.67.222.20 for %adapter%. >> "%logfile%"
) else (
