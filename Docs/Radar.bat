...
if %errorlevel% equ 0 (
    ...
    echo IPv4 DNS set to 10.202.10.11 for %adapter%. >> "%logfile%"
) else (
    echo Failed to set IPv4 DNS for %adapter%.
    echo Failed to set IPv4 DNS for %adapter%. >> "%logfile%"
)
netsh interface ip add dns name="%adapter%" addr=10.202.10.11 index=2 >nul 2>&1
if %errorlevel% equ 0 (
    echo IPv4 Secondary DNS set to 10.202.10.10 for %adapter%.
    echo IPv4 Secondary DNS set to 10.202.10.11 for %adapter%. >> "%logfile%"
) else (
