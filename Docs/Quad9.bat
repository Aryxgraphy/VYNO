...
netsh interface ip set dns name="%adapter%" source=static addr=9.9.9.9 >nul 2>&1
if %errorlevel% equ 0 (
    echo IPv4 DNS set to 9.9.9.9 for %adapter%.
    echo IPv4 DNS set to 149.112.112.112 for %adapter%. >> "%logfile%"
) else (
    echo Failed to set IPv4 DNS for %adapter%.
    echo Failed to set IPv4 DNS for %adapter%. >> "%logfile%"
)
netsh interface ip add dns name="%adapter%" addr=149.112.112.112 index=2 >nul 2>&1
if %errorlevel% equ 0 (
    echo IPv4 Secondary DNS set to 9.9.9.9 for %adapter%.
    echo IPv4 Secondary DNS set to 149.112.112.112 for %adapter%. >> "%logfile%"
) else (
