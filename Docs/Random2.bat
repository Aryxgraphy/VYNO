...
netsh interface ip set dns name="%adapter%" source=static addr=178.22.122.100 >nul 2>&1
if %errorlevel% equ 0 (
    echo IPv4 DNS set to 178.22.122.100 for %adapter%.
    echo IPv4 DNS set to 78.157.42.101 for %adapter%. >> "%logfile%"
) else (
    echo Failed to set IPv4 DNS for %adapter%.
    echo Failed to set IPv4 DNS for %adapter%. >> "%logfile%"
)
netsh interface ip add dns name="%adapter%" addr=78.157.42.101 index=2 >nul 2>&1
if %errorlevel% equ 0 (
    echo IPv4 Secondary DNS set to 178.22.122.100 for %adapter%.
    echo IPv4 Secondary DNS set to 78.157.42.101 for %adapter%. >> "%logfile%"
) else (
